			       ...
			       uint32_t             *args)
{
	...
	tmp_buf_size = new_sp - sram_info->scratch_addr;
	...
	aw_fel_write(usb, tmp_buf, sram_info->scratch_addr, tmp_buf_size);
	free(tmp_buf);
	return true;
}
...
bool aw_fel_remotefunc_execute(libusb_device_handle *usb, uint32_t *result)
{
	...
	if (result) {
		aw_fel_read(usb, sram_info->scratch_addr + 0x48, result, sizeof(uint32_t));
